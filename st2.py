...
class St2(BotPlugin):
    ...
    def st2_execute_actionalias(self, msg, match):
        ...
        if st2token is False:
            rejection = "Error: {}  Action-Alias execution is not allowed for chat user '{}'." \
                "  Please authenticate using {}session_start or see your StackStorm administrator to grant access" \
                ".".format(self.cfg.plugin_prefix, err_msg, user_id)
    ...
    def dynamic_commands(self):
        ...
        self.create_dynamic_plugin(
            name="St2",
            doc="StackStorm plugin for authentication and Action Alias execution."
                "  Use {}{}help for action alias"
                " help.".format(self.cfg.bot_prefix, self.cfg.plugin_prefix),
            commands=(
                Command(
